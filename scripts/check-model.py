...
with tempfile.TemporaryDirectory() as temp:
    ...
    with contextlib.redirect_stdout(output):
        exec((root/'public/python/sensitivity.py').read_text(),scope)
        exec((root/'public/python/analyze.py').read_text(),scope)
    ...
    assert posterior['sensitivity']['status'] == 'available', output.getvalue()
    assert len(posterior['sensitivity']['groups']['prior']) == 21
    np.testing.assert_allclose([c['median'] for c in posterior['sensitivity']['groups']['prior'][10]['channels']], [c['median'] for c in posterior['contributions']])
